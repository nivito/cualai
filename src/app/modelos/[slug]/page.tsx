...
export async function generateMetadata({
  ...
  if (!model) return {};
  return {
    title: `${model.name} (${model.company}): precio, contexto y casos de uso — cual.ai`,
    description: `${model.description} Contexto: ${model.contextWindowLabel}. Precio API: ${model.apiPricing.label}.`,
    alternates: { canonical: `https://cual.ai/modelos/${slug}` },
    openGraph: {
      title: `${model.name} — ${model.company} · cual.ai`,
      description: model.description,
      url: `https://cual.ai/modelos/${slug}`,
      type: "website",
      siteName: "cual.ai",
    },
  };
...
export default async function ModeloPage({
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "cual.ai", item: "https://cual.ai" },
      { "@type": "ListItem", position: 2, name: "Modelos", item: "https://cual.ai/modelos" },
      { "@type": "ListItem", position: 3, name: model.name, item: `https://cual.ai/modelos/${slug}` },
    ],
  };

  const faqItems = [
    { q: `¿Cuánto cuesta ${model.name}?`, a: `API: ${model.apiPricing.label}. ${model.userPricing.paidLabel ? `Plan de usuario: ${model.userPricing.paidLabel}.` : model.userPricing.free ? "Disponible gratis via web." : ""}` },
    { q: `¿Cuántos tokens de contexto tiene ${model.name}?`, a: `${model.name} tiene una ventana de contexto de ${model.contextWindowLabel} (~${Math.round(model.contextWindow * 0.75).toLocaleString()} palabras).` },
    { q: `¿Para qué es mejor ${model.name}?`, a: model.bestFor.join(", ") + "." },
    { q: `¿${model.name} es open source?`, a: model.available.includes("self-hosted") ? `Sí, ${model.name} está disponible para self-hosting.` : `No, ${model.name} solo está disponible via API o interfaz web.` },
  ];
          ...
          <section className="py-8 px-4">
            <div className="max-w-3xl mx-auto">
              ...
              <nav className="text-[10px] text-text-muted mb-4 flex items-center gap-1">
                ...
                <span>/</span>
                <Link href="/modelos" className="hover:text-accent transition-colors">Modelos</Link>
                <span>/</span>
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {model.isNew && (
                      <span className="text-[10px] px-2 py-0.5 rounded bg-accent/20 text-accent font-bold">NUEVO</span>
                    )}
                  </div>
                  <p className="text-text-muted text-xs mt-0.5">
                    {model.company} · Lanzado {model.releasedAt}
                  </p>
                  <div className="flex flex-wrap gap-1 mt-2">
                    {model.typeLabels.map((t) => (
                      <span key={t} className="text-[10px] px-1.5 py-0.5 rounded border border-border text-text-muted">{t}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                <div className="border border-border rounded bg-bg-card p-4">
                  <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">Contexto</p>
                  <p className="text-sm font-bold font-mono">{model.contextWindowLabel}</p>
                </div>
                <div className="border border-border rounded bg-bg-card p-4">
                  <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">Velocidad</p>
                ...
                <div className="border border-border rounded bg-bg-card p-4">
                  <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">Inteligencia</p>
                ...
                <div className="border border-border rounded bg-bg-card p-4">
                  <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">Disponible en</p>
              ...
              {/* Precios */}
              <div className="border border-border rounded bg-bg-card p-6 mb-4">
                <h2 className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-4">// Precios</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">API (por M tokens)</p>
                    <p className="text-sm font-bold">{model.apiPricing.label}</p>
                    <p className="text-[10px] text-text-muted mt-1">entrada / salida</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-text-muted mb-1">Plan de usuario</p>
                    {model.userPricing.free ? (
                      <p className="text-sm font-bold text-green">Gratis disponible</p>
                    ) : (
                      <p className="text-sm font-bold">{model.userPricing.paidLabel ?? "Solo API"}</p>
                    )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                <div className="border border-border rounded bg-bg-card p-5">
                  <h2 className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-3">// Fortalezas</h2>
                ...
                <div className="border border-border rounded bg-bg-card p-5">
                  <h2 className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-3">// Limitaciones</h2>
                  <ul className="space-y-1.5">
              ...
              <div className="border border-border rounded bg-bg-card p-5 mb-4">
                <h2 className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-3">// Mejor para</h2>
              ...
              <div className="border border-border rounded bg-bg-card p-5 mb-6">
                <h2 className="text-xs font-semibold uppercase tracking-widest text-text-muted mb-4">// Preguntas frecuentes</h2>
